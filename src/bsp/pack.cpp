...
namespace bsp
{
    ...
    {
        out = pack_result{};
        const bool zip = extension(output) == ".zip";
        ...
        out.res_path = zip
            ? generated_relative
            : (stdfs::path(output) / generated_relative).string();
        ...
        {
            std::error_code ec;
            if (stdfs::is_directory(stdfs::path(output), ec))
            {
                set_error(error, "ZIP output is a directory: '" + output + "'");
                return false;
            }
            if (fs::exists(output) && !options.force)
            {
                set_error(error, "ZIP archive already exists: '" + output
                          + "' (use -force to overwrite)");
                return false;
            }

            std::vector<format::zip_entry> entries;
            entries.reserve(jobs.size() + 1);
            for (const copy_job &job : jobs)
            {
                ...
                entries.emplace_back(job.relative, std::move(data));
                out.copied++;
            }
            entries.emplace_back(
                generated_relative,
                std::vector<byte>(res_text.begin(), res_text.end()));

            std::vector<byte> archive;
            std::string zip_error;
            if (!format::create_zip(entries, archive, &zip_error))
            {
                set_error(error, "could not create ZIP archive: " + zip_error);
                return false;
            }
            std::string parent = fs::directory(output);
            if (!parent.empty() && !fs::make_directory(parent))
            {
                set_error(error, "could not create ZIP output directory '"
                          + parent + "'");
                return false;
            }
            if (!fs::write_all(output, archive.data(), archive.size()))
            {
                set_error(error, "could not write ZIP archive '" + output + "'");
                return false;
            }
            return true;
        }
